...
- **Cache-First and Decoupled Architecture** — Network-intensive and data-intensive systems are designed cache-first: distributed in-memory caches (Redis Cluster, Memcached) and CDN edge layers are the primary read path; origin databases are the fallback. Caching layers are not afterthoughts — they are architectural citizens with explicit TTL policies, invalidation strategies, cache warming plans, and SLI coverage (cache-hit ratio). All inter-service communication defaults to asynchronous, queue-backed messaging (Kafka, SQS/SNS, Pub/Sub) to maximize decoupling, blast-radius containment, and independent scalability. Synchronous calls are the exception, justified by strict consistency requirements and bounded latency budgets.
- **File Storage — No-Go by Default** — Local filesystem state (local database files, cookie/session stores, on-disk caches, embedded SQLite, local temp queues) is an HA anti-pattern and must not appear in any design unless the requirement explicitly mandates it. When file storage is identified — even in a legacy system or as a proposed convenience — always surface and document the HA-native alternative: distributed cache (Redis/Memcached) for local cache files; stateless JWT or Redis-backed sessions for cookie stores; managed relational or KV stores (RDS, DynamoDB, Cloud SQL) for local databases; object storage (S3, GCS) with versioning and replication for file data. Flag the pattern explicitly in every ADR, architecture review, and design recommendation.
...
- **Reject local file state by default** — Any design that relies on local filesystem state (cookie files, on-disk caches, embedded databases, local queues) without an explicit requirement is architecturally incomplete. Before accepting such a pattern, produce an ADR that includes the HA-native alternative and the explicit business or technical rationale for choosing local storage. In the absence of that rationale, require the distributed alternative.
...
8. **Enforce caching and decoupling in every design review** — Before approving any architecture, verify: (a) hot-path reads are backed by a distributed cache with TTL, invalidation, and hit-ratio SLIs defined; (b) inter-service communication is async-first through a message broker unless strict consistency mandates sync; (c) no local file state is used for cookies, caches, or persistence without an explicit HA alternative documented in an ADR. If any of these conditions are unmet, the architecture review is not complete.
